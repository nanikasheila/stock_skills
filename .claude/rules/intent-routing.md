...
| **保有管理** | 持ち株について操作・確認したい | PF、ポートフォリオ、買った、売った、損益、ヘルスチェック、CSVインポート |
...
| **セキュリティ** | 個人情報・機密情報の漏洩をチェックしたい | 個人情報、セキュリティ、漏洩、チェック、秘密、APIキー、メアド |
...
| **可視化**: 推移を見せて、グラフ、ダッシュボード、チャート | → `/portfolio-dashboard` |
| **ドローダウン**: ドローダウン、下落率、最大下落 | → `/portfolio-dashboard` |
| **リスク可視化**: シャープレシオ推移、ローリングシャープ、リスク推移 | → `/portfolio-dashboard` |
| **相関分析可視化**: 相関ヒートマップ、銘柄間の相関、相関マトリクス | → `/portfolio-dashboard` |
| **構成可視化**: ツリーマップ、構成比、ウェイトドリフト、配分ずれ | → `/portfolio-dashboard` |
| **ベンチマーク比較**: ベンチマーク、VTI対比、インデックス比較 | → `/portfolio-dashboard` |
| **パフォーマー確認**: top/worst、最高/最低リターン銘柄 | → `/portfolio-dashboard` |
| **ヘルスチェック可視化**: ダッシュボードでヘルスチェック、売りタイミング、売り通知、アラート一覧 | → `/portfolio-dashboard`（ヘルスチェックセクション） |
| **経済ニュース**: 経済ニュース、PF影響、マクロニュース、ニュースの影響 | → `/portfolio-dashboard`（経済ニュース & PF影響セクション） |
| **CSVインポート**: 取引履歴CSV、インポート、取り込み | → `scripts/import_trade_csv.py` |
...
**CSVインポート（取引履歴の一括取り込み）**:

ユーザーがCSVファイルのパスを提示して「取り込んで」「インポートして」と言った場合、
`scripts/import_trade_csv.py` を実行する。楽天証券の取引履歴CSV（JP/US形式）に対応。

| ユーザーの表現 | 実行コマンド |
|:---|:---|
| 「このCSVを取り込んで」+ ファイルパス | `python3 scripts/import_trade_csv.py <ファイルパス>` |
| 「取引履歴をインポートして」+ ファイルパス | `python3 scripts/import_trade_csv.py <ファイルパス>` |
| 「tradehistory(JP)を取り込んで」 | `python3 scripts/import_trade_csv.py data/history/trade/tradehistory(JP)_*.csv` |
| 「取引CSVを全部インポート」 | `python3 scripts/import_trade_csv.py data/history/trade/` |
| 「まず中身を確認して」 | `python3 scripts/import_trade_csv.py --dry-run <ファイルパス>` |

- ファイルパスが会話中に添付またはテキストで提示されていれば、そのパスを使用する
- ディレクトリを指定すると中の全CSVを一括処理
- `--dry-run` で保存せず解析結果のみ表示（件数・集約結果・エラーの確認）
- 同名JSONが既にある場合は自動スキップ（重複防止）
- 同日・同銘柄・同方向の取引は加重平均で自動集約
...
### セキュリティドメイン → `/security-check`

```
「個人情報が含まれてないかチェック」    → /security-check --verbose
「セキュリティチェックして」           → /security-check --verbose
「APIキー漏れてない？」              → /security-check --verbose
「gitに秘密情報ある？」              → /security-check --verbose
「メアドが漏れてないか確認」          → /security-check --verbose
「リポジトリの安全性を確認」          → /security-check --verbose
```

**判定キーワード**: 個人情報、セキュリティ、漏洩、漏れ、秘密、機密、APIキー、メアド、メールアドレス、パスワード、安全性、チェック
...
📥 取引CSV取り込み
  「このCSVを取り込んで」「取引履歴をインポートして」
  → 楽天証券CSV（JP/US）を自動判定・集約してJSONに変換
  → 同日同銘柄の取引は加重平均で自動集約、重複スキップ
...
📊 ポートフォリオダッシュボード
  「PFの推移を見せて」「グラフで見たい」「ダッシュボード」
  「ドローダウンを見たい」「シャープレシオの推移」「相関ヒートマップ」
  「ツリーマップ」「ウェイトドリフト」「ベンチマーク比較」
  「top/worstパフォーマー」
  「ダッシュボードでヘルスチェック」「売りタイミング通知」「アラート一覧」
  「経済ニュース」「PFへの影響」「マクロニュース」
  → 資産推移・リスク指標・ドローダウン・ローリングシャープ・
    ベンチマーク超過リターン・ツリーマップ・相関ヒートマップ・
    ウェイトドリフト警告・ヘルスチェック・売りタイミング通知・
    経済ニュース&PF影響分析を
    ブラウザで可視化

🔒 セキュリティチェック
  「個人情報が含まれてないかチェック」「APIキー漏れてない？」
  → Git履歴・追跡ファイルの個人情報・機密情報を自動検出
  → メールアドレス・実名・APIキー・パスワード・絶対パスを検出
```
