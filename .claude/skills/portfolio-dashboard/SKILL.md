...
- **影響度サマリーカード**: 🔴高影響 / 🟡中影響 / 🔵低影響 / ⚪影響なしの件数を4列カードで表示
- **PF影響ありのニュース**: 影響度順に表示。各ニュースに影響カテゴリバッジ（🏦金利・💱為替・🌍地政学・📊景気・💻テック・⛽エネルギー）と影響銘柄リストを付与
- **その他のニュース**: PF影響なしのニュースは折り畳み表示
- **ニュース取得元**: S&P500, 日経平均, NYダウ, 米10年債, 原油(WTI), 金, USD/JPY の主要7指標から自動取得
- **影響度判定ロジック**: ニュースタイトルのキーワード分析 × 保有銘柄のセクター・通貨・ベータから自動推定
  - 保有銘柄の50%以上が影響対象 → 🔴高影響
  - 保有銘柄の20%以上が影響対象 → 🟡中影響
  - 一部が影響対象 → 🔵低影響
...
- `fetch_economic_news(positions, fx_rates)`: 主要7指数のニュースを取得しPF影響を分析
  - `_classify_news_impact(title)`: ニュースタイトルからカテゴリを推定（6カテゴリ: 金利/為替/地政学/景気/テック/エネルギー）
  - `_estimate_portfolio_impact(categories, positions, fx_rates)`: カテゴリ×保有銘柄で影響度を推定（high/medium/low/none）
